# START HERE — forking this project into a new chat window

Paste this as your first message in the new session:

---

Read HANDOFF.md in this folder and continue the get-an-expert marketplace
work. Current state: Phase A wireframe approved, Direction A brand locked
(serif headlines, NO italics), home tiles + UGC detail page built in hifi/.
Next up: my review of the revised pages, then the remaining 5 detail pages.

---

## What's in this bundle

- `HANDOFF.md` — the single source of truth. Positioning, all 6 SKUs,
  progress log, locked decisions, open questions, build phases, copy rules.
- `hifi/index.html` — marketplace home, hero + 6 SKU tiles (Direction A).
- `hifi/ugc.html` — complete UGC Ads detail page (the template for all SKUs).
- `hifi/directions.html` — the 3 brand directions explored (A chosen).
- `hifi/shots/` — screenshots taken so far.
- `wireframe/` — Phase A low-fi wireframes (home + UGC) and their shots.
- `marketplace-tiles-plan.md` — session-1 plan, historical context only.

## What is NOT in this bundle (and where it lives)

- The Kimi v3 homepage design preview: "Kimi_Agent_Midsesh.com Enhancement
  Plan (1).zip" in ~/Downloads (extract to `design/` if a session needs it).
- The real Next.js site: clone https://github.com/Pulkitwalia099/get-an-expert-web
  into `repo/` when the port phase starts.

## Ground rules for any session

1. Read HANDOFF.md first. Update its progress log before ending the session.
2. All artifacts stay in this folder.
3. DESIGN SYSTEM IS FROZEN: fonts stay the original site font (system sans
   stack), theme/tokens/skeletons stay as they are. Do not change them unless
   the user explicitly decides. Copy comes from Rohit where he has written it.
4. Copy rules: terse, no em dashes, no exclamation marks, no hype words,
   NO italics. Emoji only where the user explicitly asked (UGC How-it-works).
5. Prices/copy marked TBD must stay visibly marked until the user decides.
